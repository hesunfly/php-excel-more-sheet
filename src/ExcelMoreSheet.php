...
namespace Hesunfly\ExcelMoreSheet;
...
//        ob_start();
        ...
        foreach ($list as $sheet_key => $sheet_item) {
            if ($sheet_key > 0) {
                $spreadsheet->createSheet();
                $spreadsheet->setActiveSheetIndex($sheet_key);
            }

            $sheet = $spreadsheet->getActiveSheet();
            // 写入头部
            $hk = 1;
            $header = $sheet_item['header'];
            foreach ($header as $k => $v) {
                $sheet->setCellValue(Coordinate::stringFromColumnIndex($hk) . '1', $v[0]);
                $hk += 1;
            }

            $lists = $sheet_item['list'];
            // 开始写入内容
            $column = 2;
            $size = ceil(count($lists) / 500);
            for ($i = 0; $i < $size; $i++) {
                $buffer = array_slice($lists, $i * 500, 500);
            ...
            $sheet->setTitle($sheet_item['title']);
        }
